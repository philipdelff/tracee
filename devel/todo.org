...
*** TODO misc arg 
** ggwrite
*** TODO [#A] canvas should be last in file name
...
** ftwrite
*** TODO arg to set caption font size etc?
Or is that just ft.fun?
*** TODO Caption shout reuse background from main
In stead of default to white
...
** parameter functions
- include createParameterTable and printParameterTable from NMwork
New name for createParameterTable? "create" may be uninformative? Could
be NMparameterTable() NMparTab() 
It would make sense to create a print method for simply:
> print(createParameterTable(file.mod))
would give an R console parameter table
